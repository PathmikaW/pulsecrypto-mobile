...
- **Mobile — Jest, 14 suites, 71 tests, all passing.** Unit tests cover the REST error/retry layer of ADR-M12 (`backoff`, `errors`, `httpClient` via an injected adapter, and the retry policy through a real `QueryClient`), `WebSocketSource` (backoff, silence timeout), `MarketRepository`, `createThrottledStorage`, `FavouritesRepository`, `useWatchlist` (tracked/untracked merge, search, fallback to cached data, favourite toggle), the mapper and the formatting utilities.
