...
### Delivery status (v9.1) — what is actually done
...
**Not done:** CI for either repository, including the contracts-drift check as a CI step (a manual `pnpm run check:contracts` exists — ADR-X1, ADR-X3); the measured FlashList-vs-FlatList comparison (ADR-M3); React Native Testing Library component tests, including the non-English-locale render check (§8); the merge of `release/v1.0.0` (already cut in both repositories) into `main`, and the `v1.0.0` tag (ADR-X2); the screen recording (§10) — a deliverable only the developer can produce.
