...
**Done and verified:** both scaffolds (ADR-X5); backend ingestion, conflation, backpressure, WebSocket server, `/pairs/meta`, `/health`, `/metrics`; the shared contract, mirrored byte-for-byte; mobile foundation, all five screens (Terminal, Markets, Telemetry, Settings sharing one screen, account drawer), favourites, search, price-flash and order-book animations, offline behaviour with automatic reconnection, pull-to-refresh, WS message validation and the error boundary; a common REST error model with an axios client and a shared retry policy (ADR-M12); the Bottom Tabs / focus-gated-subscription performance work (ADR-M11); the mobile app built and run on the Android Emulator; unit and integration tests in both repositories; both READMEs.

**Not done:** CI for either repository, including the contracts-drift check as a CI step (a manual `pnpm run check:contracts` exists — ADR-X1, ADR-X3); Docker build and run verification (Docker is not installed on the development machine — ADR-X4); the measured FlashList-vs-FlatList comparison (ADR-M3); React Native Testing Library component tests, including the non-English-locale render check (§8); the `release/v1.0.0` branch, the merge into `main` and the tag (ADR-X2); the screen recording (§10) — a deliverable only the developer can produce.
