...
- **Scaffold each project manually, using each framework's official setup command, per ADR-X5 — not generated by an AI coding session.** Run the backend's Fastify generator, and the mobile app's `create-expo-app` + `expo prebuild`, confirm each starts/builds correctly, and commit the verified scaffold _before_ handing either repo to an AI coding session for feature implementation
...
### Delivery status (v9.5) — what is actually done

**Done and verified:** both scaffolds (ADR-X5); backend ingestion, conflation, backpressure, WebSocket server, `/pairs/meta`, `/health`, `/metrics`; the shared contract, mirrored byte-for-byte; mobile foundation, all five screens (Terminal, Markets, Telemetry, Settings sharing one screen, account drawer), favourites, search, price-flash and order-book animations, offline behaviour with automatic reconnection, pull-to-refresh, WS message validation and the error boundary; a common REST error model with an axios client and a shared retry policy (ADR-M12); the Bottom Tabs / focus-gated-subscription performance work (ADR-M11); the backend Docker image built and run on a Linux VM (ADR-X4); the mobile app built and run on the Android Emulator; a shareable arm64 release APK built with EAS and tested on a physical phone against the AWS-hosted backend (EC2 + Caddy + DuckDNS); the Gitflow release — `release/v1.0.0` merged into `main`, tagged `v1.0.0` and merged back into `develop` in both repositories (ADR-X2); unit and integration tests in both repositories; both READMEs.

**Not done:** CI for either repository, including the contracts-drift check as a CI step (a manual `pnpm run check:contracts` exists — ADR-X1, ADR-X3); the measured FlashList-vs-FlatList comparison (ADR-M3); React Native Testing Library component tests, including the non-English-locale render check (§8); the screen recording (§10) — a deliverable only the developer can produce.

_The above is a target sequence, not a fixed commitment — mobile native-build friction, even on Expo, tends to consume unpredictable time regardless of planning._
